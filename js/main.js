...
// SEARCH TOGGLE (Header Search Bar remains intact - Option 1)
const searchIcon = document.querySelector('.search-icon');
const searchInput = document.querySelector('.search-input');
searchIcon.addEventListener('click', () => {
// Toggle visibility of search input; the icon remains visible in the header
searchInput.style.display = (searchInput.style.display === 'block') ? 'none' : 'block';
if (searchInput.style.display === 'block') searchInput.focus();
});

// DYNAMIC SEARCH FUNCTIONALITY (Option 1)
document.addEventListener("DOMContentLoaded", function() {
const searchResultsSection = document.getElementById('searchResults');
const searchResultsContent = document.getElementById('searchResultsContent');

// Default product sections that will be hidden when search is active
const catProducts = document.getElementById('catProducts');
const dogProducts = document.getElementById('dogProducts');
  ...
  if (query.length > 0) {
    fetch('search.php?search=' + encodeURIComponent(query))
      ...
      .then(html => {
        ...
        // Hide default category sections while searching
        if (catProducts) catProducts.style.display = "none";
        if (dogProducts) dogProducts.style.display = "none";
      })
      .catch(error => {
        console.error('Error fetching search results:', error);
      });
  } else {
    // If the search input is cleared, hide the search results and restore default sections
    searchResultsSection.style.display = "none";
    searchResultsContent.innerHTML = "";
    if (catProducts) catProducts.style.display = "block";
    if (dogProducts) dogProducts.style.display = "block";
  }
});
});
